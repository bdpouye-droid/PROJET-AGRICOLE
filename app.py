...
def init_db():
    ...
    # inbox entries: simple actionable items
    cur.execute("""
...
def inject_css():
    st.markdown("""
    <style>
      :root { --accent:#0d6efd; --muted:#6c757d; --bg:#f8f9fa; --card:#ffffff; }
      .stApp { background: var(--bg); font-family: "Inter", "Segoe UI", sans-serif; }
      ...
      .card { background: var(--card); padding:14px; border-radius:8px; box-shadow:0 1px 4px rgba(16,24,40,0.08); }
...
def page_audit():
    ...
    st.markdown("Journal des actions et suivi de la chaîne de vie par ticket.")
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM logs_audit ORDER BY created_at DESC LIMIT 200")
    logs = cur.fetchall()
    for log in logs:
        st.markdown(f"- [{log['created_at'][:19]}] **{log['ticket_id']}** — {log['action']} par {log['user']} — {log['details']}")
    st.markdown("---")
    st.markdown("Rechercher la chaîne de vie d'un ticket")
    ticket_search = st.text_input("Ticket (#TICK-0001)", value=st.session_state.get('selected_ticket') or "")
    if st.button("Afficher la chaîne"):
...
def page_detail():
    ...
    if role == "Achats":
            ...
            if sub:
                ...
                if action == "Refuser (nécessite motif)":
                    if not motif.strip():
                        ...
                        update_demande(ticket, status="Refusé Achats", motif_refus=motif, fournisseur_retenu=fournisseur_retenu or None)
                    ...
                    # must set fournisseur_retenu if blank -> warn but allow empty if not required by user
                    update_demande(ticket, status="Approuvé Achats", fournisseur_retenu=fournisseur_retenu or None)
                    add_log(ticket, role, "Approuvé Achats", f"Fournisseur retenu: {fournisseur_retenu or '—'}")
                    ...
                    update_demande(ticket, status="Demande Correction - Achats")
                    ...
                    update_demande(ticket, fournisseur_retenu=fournisseur_retenu or None)
                    add_log(ticket, role, "Proposition fournisseur", f"Proposition: {fournisseur_retenu or '—'}")
        ...
        st.markdown("##### Vue DG (Lecture & Suivi seulement)")
        st.info("La DG a un accès de suivi global. Les actions de validation finale sont réalisées après le circuit normal par Achats et Finance.")
        if st.button("Demander point / commenter"):
            comment = st.text_area("Votre commentaire (sera enregistré dans le journal)")
            if comment:
                add_log(ticket, role, "Commentaire DG", comment)
        ...
        if st.session_state['current_user'] == r['emitter'] or st.session_state['current_user'] == "Admin":
            ...
            if r['status'] in ("Demande Correction - Achats", "Demande Correction - Finance"):
